...
function TakeOutVehicle(vehicleInfo)
    ...
    QBCore.Functions.SpawnVehicle(vehicleInfo, function(veh)
        SetVehicleNumberPlateText(veh, "Highways"..tostring(math.random(1000, 9999)))
...
function MenuGarage()
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "highways:client:TakeOutVehicle",
                args = {
                    vehicle = veh
                }
            }
...
RegisterNetEvent('highways:client:TakeOutVehicle', function(data)
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)
    PlayerJob = JobInfo
    TriggerServerEvent("ambulance:server:Sethighways")
end)
...
CreateThread(function()
    ...
    while true do
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "highways" then
                    ...
                    if onDuty then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "highwaysstash_"..QBCore.Functions.GetPlayerData().citizenid)
                                TriggerEvent("inventory:client:SetCurrentStash", "highwaysstash_"..QBCore.Functions.GetPlayerData().citizenid)
                            end
                                ...
                                if IsControlJustReleased(0, 38) then
                                    if IsPedInAnyVehicle(ped, false) then
                                        ...
                                        QBCore.Functions.SpawnVehicle(Config.Helicopter, function(veh)
                                            ...
                                            SetEntityHeading(veh, coords.w)
                                            SetVehicleLivery(veh, 1) -- highways Livery
                                            exports['LegacyFuel']:SetFuel(veh, 100.0)
